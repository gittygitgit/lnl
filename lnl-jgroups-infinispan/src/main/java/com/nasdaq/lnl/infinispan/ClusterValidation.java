...
public class ClusterValidation {
	...
	public static boolean waitForClusterToForm(EmbeddedCacheManager cacheManager, int nodeId, int clusterSize) {
		return new ClusterValidation(cacheManager.getCache(), nodeId, clusterSize).checkReplicationSeveralTimes() > 0;
	}
	...
	private void tryToPut() {
		...
		while (tryCount < 5) {
			try {
				System.out.println("Putting node check [nodeId=" + nodeId + "]");
	...
	private int replicationCount(int clusterSize) {
		...
		System.out.println("Replication count=" + replicaCount);
		return replicaCount;
	}
