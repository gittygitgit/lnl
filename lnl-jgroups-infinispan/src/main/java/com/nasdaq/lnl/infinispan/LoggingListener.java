...
public class LoggingListener {
   ...
   public void observeAdd(CacheEntryCreatedEvent<?, ?> event) {
      if (!event.isPre()) // So that message is only logged after operation succeeded
         log.infof("Cache entry [key=%-5s, value=%20s] added in cache %s", event.getKey(), event.getCache().get(event.getKey()), event.getCache());
   }

   @CacheEntryRemoved
   public void observeRemove(CacheEntryRemovedEvent<?, ?> event) {
      log.infof("Cache entry [key=%-5s, value=%20s]] removed from cache %s", event.getKey(), event.getValue(), event.getCache());
   }
